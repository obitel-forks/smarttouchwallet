package org.vikulin.smarttouchwallet.icon;
...
public class Blockies {
    ...
    public Blockies(int size, String seed, int scale) {
        createIcon(size, seed, scale);
    }

    public static Bitmap createIcon(int size, String address){
        return createIcon(size, address, 16);
    }

    public static Bitmap createIcon(int size, String address, int scale){
        seedrand(address);
        HSL color = createColor();
        HSL bgColor = createColor();
        HSL spotColor = createColor();

        double [] imgdata = createImageData(size);
    ...
    private static double [] createImageData(int size){
        int width = size;
        int height = size;
